...
Sub displayQueryTable(Optional sTbl = "", Optional sSQL = "", Optional spath = "", Optional tblType = "", Optional top = "A4", Optional bWriteInfo = True)
...
Sub displayParamQuery(Optional sProc = "", Optional sSQL = "", Optional prmValAry, Optional spath = "", Optional top = "A4", Optional bWriteInfo = True)
    Dim qt
    Dim sn, sCon, sSQL0
    Dim infoAry, r, c
    Dim cmd, rst
...
Function aggIf(exp, cond, Optional agg = "max")
    Dim ret
    ret = agg & "(iif(" & cond & "," & exp & ",null))"
    aggIf = ret
End Function
